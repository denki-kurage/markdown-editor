import { MarkdownContentBase, MarkdownRange } from '@mde/markdown-core';
import { TablePosition } from './TablePosition';
import { Direction } from './Direction';
import { MarkdownAlignments } from './MarkdownAlignments';
export class TablePositionDirections {
    static top = new TablePosition(-1, 0);
    static bottom = new TablePosition(1, 0);
    static left = new TablePosition(0, -1);
    static right = new TablePosition(0, 1);
    static before = 0;
    static after = 1;
...
export class TableCellInfo {
    ...
    getCellFromRelative(nextPosition) {
        return TableCellInfo.createInstanceFromTablePosition(this.table, this.tablePosition.newAdded(nextPosition));
    }
    /**
...
export class TableAlignmentCell extends TableCell {
    _align = MarkdownAlignments.Left;
    _alignWord = '---';
    get align() {
        return this._align;
    }
...
export class TableRowBase {
    ...
    *[Symbol.iterator]() {
     ...
     * @param strCount
     */
    getCellInfomationFromCharacterIndex(charIndex, strCounter) {
        for (const r of this.getCellInfomations(strCounter)) {
    ...
    getCellInfomationFromColumnIndex(columnIndex, strCounter) {
        for (const r of this.getCellInfomations(strCounter)) {
    ...
    *getCellInfomations(strCounter = str => str.length) {
        let cp = 0;
