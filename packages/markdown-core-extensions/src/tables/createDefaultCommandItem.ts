import { ICommand, ICommandItem } from '@kurage/markdown-core';
...
type mapsKeysType = keyof typeof maps;
const mapsKeys = Object.keys(maps) as mapsKeysType[];
...
export const createDefaultCommandItem: (table: MarkdownTable, commands: Map<string, ICommand>, mode: 'light' | 'dark') => ICommandItem =
    (table, commands, mode) =>
{
    const iconsMap: any = mode === 'light' ? lightIconsList : darkIconsList;

    const children = mapsKeys.map((group) =>
    {
